// This file is auto-generated by scripts/generate.ts
// Public SDK exposes only API key-authenticated endpoints

export { search, getSearchStats, searchWords, getStatsOverview, getCoveredWords, listMedia, getSegmentByUuid, getSegmentContext, listSeries, getSeries, getMedia, listEpisodes, getEpisode, getSegment, type Options } from './sdk.gen';
export type {
  SearchData,
  SearchErrors,
  SearchError,
  SearchResponses,
  SearchResponse,
  GetSearchStatsData,
  GetSearchStatsErrors,
  GetSearchStatsError,
  GetSearchStatsResponses,
  GetSearchStatsResponse,
  SearchWordsData,
  SearchWordsErrors,
  SearchWordsError,
  SearchWordsResponses,
  SearchWordsResponse,
  GetStatsOverviewData,
  GetStatsOverviewErrors,
  GetStatsOverviewError,
  GetStatsOverviewResponses,
  GetStatsOverviewResponse,
  GetCoveredWordsData,
  GetCoveredWordsErrors,
  GetCoveredWordsError,
  GetCoveredWordsResponses,
  GetCoveredWordsResponse,
  ListMediaData,
  ListMediaErrors,
  ListMediaError,
  ListMediaResponses,
  ListMediaResponse,
  GetSegmentByUuidData,
  GetSegmentByUuidErrors,
  GetSegmentByUuidError,
  GetSegmentByUuidResponses,
  GetSegmentByUuidResponse,
  GetSegmentContextData,
  GetSegmentContextErrors,
  GetSegmentContextError,
  GetSegmentContextResponses,
  GetSegmentContextResponse,
  ListSeriesData,
  ListSeriesErrors,
  ListSeriesError,
  ListSeriesResponses,
  ListSeriesResponse,
  GetSeriesData,
  GetSeriesErrors,
  GetSeriesError,
  GetSeriesResponses,
  GetSeriesResponse,
  GetMediaData,
  GetMediaErrors,
  GetMediaError,
  GetMediaResponses,
  GetMediaResponse,
  ListEpisodesData,
  ListEpisodesErrors,
  ListEpisodesError,
  ListEpisodesResponses,
  ListEpisodesResponse,
  GetEpisodeData,
  GetEpisodeErrors,
  GetEpisodeError,
  GetEpisodeResponses,
  GetEpisodeResponse,
  GetSegmentData,
  GetSegmentErrors,
  GetSegmentError,
  GetSegmentResponses,
  GetSegmentResponse
} from './types.gen';

// Common entity types
export type {
  Segment,
  Media,
  Episode,
  Series,
  Character,
  Seiyuu,
  PaginationInfo,
  OpaqueCursorPagination
} from './types.gen';

// Re-export client factory
export { createNadeshikoClient, type NadeshikoClient, type NadeshikoConfig } from './nadeshiko.gen';

// Re-export singleton client
export { client } from './client.gen';

export type { Client, Config } from './client';

// Re-export helpers
export { paginate, type PaginationMeta } from './paginate';
export { withRetry, type RetryOptions } from './retry';
export { NadeshikoError, type NadeshikoErrorCode, type NadeshikoProblemDetails } from './errors';
